...
Board::Board(const unsigned int &size) {
  ...
  for (auto &vec : tablero) {
    ...
    for (auto &casilla : vec) {
      casilla = std::make_unique<Square>(
          Square(SQ_Color::EMPTY)); // Use Default Constructor
    }
  }
}
...
void Format(
    const P_Color &color,
    const std::vector<std::vector<std::shared_ptr<Square>>> &tablerobase,
    std::vector<std::vector<std::shared_ptr<Square>>> &returnTablero) {
  ...
  case P_Color::BLUE: {
    ...
    returnTablero[0].emplace_back(
        std::make_unique<StartSquare>(SQ_Color::BLUE));

    returnTablero.insert(returnTablero.end(), move(tmp_vec2));
    returnTablero[size + 1].emplace_back(
        std::make_unique<EndSquare>(SQ_Color::BLUE));
  ...
  case P_Color::RED: {
    ...
    for (auto it = returnTablero.begin(); it != (returnTablero.end() - 1);
         ++it) {
      it->insert(it->begin(), std::make_unique<Square>(SQ_Color::RED));
      it->insert(it->end(), std::make_unique<Square>(SQ_Color::RED));
    }

    returnTablero[size - 1].insert(
        returnTablero[size - 1].begin(),
        std::make_unique<StartSquare>(SQ_Color::RED));
    returnTablero[size - 1].insert(returnTablero[size - 1].end(),
                                   std::make_unique<EndSquare>(SQ_Color::RED));
    break;
  }
  case P_Color::NONE: {
...
bool Board::verifyConnection(const P_Color &playerColor) const {
  ...
  while (!queue.empty()) {
    ...
    std::shared_ptr<Square> top_casilla = move(queue.front());
    queue.pop();

    // if they are invalid paths (empty or enemy)
    SQ_Color top_color = top_casilla->getColor();
    if (top_color == SQ_Color::EMPTY ||
        top_color == static_cast<SQ_Color>(playerColor)) {
      continue;
    }
...
bool Board::setSquare(const unsigned int &squareX, const unsigned int &squareY,
